...
function UserTable() {
  ...
  const handleCreateUser = async (newUser) => {
    try {
      ...
      const userWithId = { ...newUser, id: users.id };
      localStorage.setItem("viewUser", JSON.stringify(userWithId));

      await axios.post(API_URL, newUserWithId);
      console.log(JSON.parse(localStorage.getItem("viewUser")));
  ...
  const handleUpdateUser = async (updatedUser) => {
    try {
      // Make sure you're using the correct user ID for the PUT request
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mb-4 flex justify-center">
        <input
          ...
          className="border border-blue-300 focus:border-blue-500 focus:ring-blue-500 rounded-md p-2 w-full md:w-1/3"
          value={searchTerm}
